...
static inline int toIndex(int x, int y, int cols) { return y * cols + x; }
...
void A_StarStrategy_t::findPath(NpcEnemy_t& npc, const World_t& world, const Player_t& player) const
{
   if (world.worldMap.empty()) return;
   ...
   {
      if (t.tileDest.y == firstRowY) ++cols;
      else break;
   }
   if (cols <= 0) return;
   ...
   std::vector<float> gScore(gridSize, std::numeric_limits<float>::infinity());
   std::vector<int>   parent(gridSize, -1);
   std::vector<char>  closed(gridSize, 0);

   // Min-heap by f = g + h
   struct PQItem { int idx; float f; };
   ...
   auto isBlocked = [&](int tx, int ty) {
      ...
      for (int yy = y0; yy <= y1; ++yy)
      {
         for (int xx = x0; xx <= x1; ++xx)
         {
            ...
            if (world.worldMap[i].tileType == TileType_t::COLLISION) return true;
         }
      }
      return false;
   };
   ...
   std::reverse(pathIdx.begin(), pathIdx.end());
      ...
      if ( npc.pathIndices.size() >= 2 )
      {
         ...
         float targetX = world.worldMap[nextIdx].tileDest.x + world.worldMap[nextIdx].tileDest.width * 0.5f;
         float targetY = world.worldMap[nextIdx].tileDest.y + world.worldMap[nextIdx].tileDest.height * 0.5f;
         Vector2 dirVec = { targetX - npc.playerPosition.x, targetY - npc.playerPosition.y };
         float len = std::sqrt( dirVec.x * dirVec.x + dirVec.y * dirVec.y );
         if ( len > 0.0001f )
         {
            dirVec.x /= len; dirVec.y /= len;
